...
```{r}
trial_df <- bind_rows(readRDS("/proj/mnhallqlab/users/michael/ridl_fmri_analysis/code/tmp_ridl.rds")) %>%
  filter(id != "540001")
...
trial_df$onset_diff <- trial_df$next_start - (trial_df$feedback_time + 1.0) # 1.0 is fixed duration of feedback event
big_delays <- which(trial_df$onset_diff > 2)
summary(trial_df$onset_diff)
hist(trial_df$onset_diff)
trial_df %>% filter(abs(onset_diff) > 1)
```
...
table(abs(trial_df$onset_diff[expected_delays] - 2.5) < .2)
```
...
as.data.frame(trial_df[expected_delays[which(abs(trial_df$onset_diff[expected_delays] - 2.5) > .2)],])
```
...
prop.table(table(abs(trial_df$onset_diff - 1) < .1))

# 17% have differences > 1.2s
prop.table(table(trial_df$onset_diff > 1.2))
...
data.frame(feedback_diff, trial_df$onset_diff, trial_df$trial_type) %>% head(n = 20)
```
